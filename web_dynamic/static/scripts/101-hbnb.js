...
$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          // Review div
          article.append('<div></div>');
          article.children(':last-child').addClass('reviews');
          article.children(':last-child').append('<h2></h2>');
          const H2 = article.children(':last-child').children(':last-child');
          H2.html('Reviews');
          H2.append('<span>show</span>');
          H2.children(':last-child').attr('data-id', data[i].id);
          H2.children(':last-child').addClass('click');
          article.children(':last-child').append('<ul></ul>');
        }
      }
    });
  });
  $(document).on('click', 'span.click', function () {
    ...
    if ($(this).text() === 'show') {
      ...
      const span = $(this);
      $.get(reviewEndpoint, function (data) {
        for (let i = 0; i < data.length; i++) {
          const H2 = span.parent();
          const DIV = H2.parent();
          const UL = DIV.children(':last-child');
          UL.append('<li><p>' + data[i].text + '</p></li>');
        }
      });
      $(this).html('hide');
    } else {
